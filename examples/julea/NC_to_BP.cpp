...
void read(std::string engine, std::string fileName)
{
    ...
    io.SetEngine(engine);
     ...
     for(const auto &var : varMap)
     {
     	...
     	netCDF::NcType type = variable.getType();//dataFile.getType(name,netCDF::NcGroup::Location::All);
     	...
     	// size_t size = sizeof("variable_") + sizeof(varCount) + 1;
     	// std::unique_ptr<char[]> buf( new char[ size ] );
     	// snprintf( buf.get(), size , "variable_%lu", varCount);
     	// std::string adiosVarName (buf.get(), buf.get() + size -1);
     	std::cout << "adiosVarName: " << adiosVarName << std::endl;

     	const std::string varType = "double";
     	// adios2::Variable<varType> var0 = io.DefineVariable<varType>("v0", {},{},{Nglobal});

     	if(typeID = 3)
     	{
     			auto test = io.DefineVariable<int32_t>(adiosVarName, {},{},{Nglobal});

     	}
     	else if(typeID = 4)
     	{
     			auto test = io.DefineVariable<float>(adiosVarName, {},{},{Nglobal});
     	...
     	switch(typeID)
     	{
     		...
     		std::cout << "test " << std::endl;
     			break;
     		case NC_CHAR:
     			// auto var = io.DefineVariable<char>(adiosVarName, {},{},{Nglobal});
     		std::cout << "test2 " << std::endl;
     			break;
     		case NC_SHORT:
     			auto var = io.DefineVariable<int8_t>(adiosVarName, {},{},{Nglobal});
     			break;
     		case NC_INT:
     			auto var = io.DefineVariable<int32_t>(adiosVarName, {},{},{Nglobal});
     			break;
     		case NC_FLOAT:
     			auto var = io.DefineVariable<float>(adiosVarName, {},{},{Nglobal});
     			break;
     	}
...
int main(int argc, char *argv[])
{
    ...
    try
    {
    	...
    	read("bp3", "_grib2netcdf-webmars-public-svc-blue-004-6fe5cac1a363ec1525f54343b6cc9fd8-ICkLWm.nc");
    }
