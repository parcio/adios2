...
/**
 * Called to guarantee that read/write are really executed and the results available.
 * [JuleaWriter::PerformPuts description]
 */
void JuleaWriter::PerformPuts()
{
    ...
    //is it actually necessary to differentiate? or is perform puts only called with sync?
    //probably not?!
    //PSEUDO: how to get actual value?
    // bool deferred = true;
    // if(deferred){
    //     return;
    // }
    ...
    for (const std::string &variableName : m_DeferredVariables)
    {
        ...
        if (type == "compound")
        {
            // not supported
            std::cout << "Julea Writer " << m_WriterRank
               << "     PerformPuts()" << "compound variable type not supported \n";
        }
...
{
    // m_DeferredVariables.init() FIXME: how to init set of strings? constructur?
    // Nothing to process from m_IO.m_TransportsParameters
    //std::cout << "JULEA ENGINE: Init Transport" << std::endl;
    if (m_Verbosity == 5)
    {
 ...
 * @param transportIndex [description]
 */
void JuleaWriter::AggregateWriteData(const bool isFinal, const int transportIndex)
{
     //DESIGN: check BP3Writer
    if (m_Verbosity == 5)
    {
        std::cout << "Julea Writer " << m_WriterRank << " AggregateWriteData\n";
        std::cout << " Data aggregation for writing not yet supported." << std::endl;
    }
    //TODO Implement!
    //
...
//     }
//     // for (std::vector<size_t>::iterator it = blockInfo.Count.begin() ; it != blockInfo.Count.end(); ++it)
//     // {
//         // std::cout << ' ' << *it;
//     // }
...
// void test_function(){
   ...
   //  test_file << "This is some test text to see if anything shows :)" << std::endl;
   //  test_file << "This is output from the SYNC version" << std::endl;
