...
void JuleaReader::PerformGets()
{
    ...
    for (const std::string &variableName : m_DeferredVariables)
    {
        ...
        if (type == "compound")
        {
            // not supported
            std::cout << "Julea Reader " << m_ReaderRank
               << "     PerformGets()" << "compound variable type not supported \n";
        }
#define declare_type(T)                                                        \
        else if (type == helper::GetType<T>())                                 \
        {                                                                      \
        ...
        // ADIOS2_FOREACH_TYPE_1ARG(declare_type) //TODO:Why is this different from Writer?
...
/**
 * Initializes variables so that InquireVariable can find the variable in the IO map.
 * Since the m_Variables is private the only solution is to define each variable with the according
 * parameters.
 */
// template <class T>
...
//         } // end switch
        ...
        switch(types[i])
        {
            ...
            //     break;
            case STRING:
                m_IO.DefineVariable<std::string>(names[i], shape, start, count, constantdims);
                break;
            case INT8:
                m_IO.DefineVariable<int8_t>(names[i], shape, start, count, constantdims);
                break;
            case UINT8:
                m_IO.DefineVariable<uint8_t>(names[i], shape, start, count, constantdims);
                break;
            case INT16:
                m_IO.DefineVariable<int16_t>(names[i], shape, start, count, constantdims);
                break;
            case UINT16:
                m_IO.DefineVariable<uint16_t>(names[i], shape, start, count, constantdims);
                break;
            case INT32:
                m_IO.DefineVariable<int32_t>(names[i], shape, start, count, constantdims);
                std::cout << "++ Julea Reader m_IO.DefineVariable() for: " << names[i] << std::endl;
                break;
            case UINT32:
                m_IO.DefineVariable<uint32_t>(names[i], shape, start, count, constantdims);
                break;
            case INT64:
                m_IO.DefineVariable<int64_t>(names[i], shape, start, count, constantdims);
                break;
            case UINT64:
                m_IO.DefineVariable<uint64_t>(names[i], shape, start, count, constantdims);
                break;
            case FLOAT:
                m_IO.DefineVariable<float>(names[i], shape, start, count, constantdims);
                std::cout << "++ Julea Reader m_IO.DefineVariable() for: " << names[i] << std::endl;
                break;
            case DOUBLE:
                m_IO.DefineVariable<double>(names[i], shape, start, count, constantdims);
                break;
            case LONG_DOUBLE:
                m_IO.DefineVariable<long double>(names[i], shape, start, count, constantdims);
                break;
            // case FLOAT_COMPLEX:
            //     //TODO
            //     break;
...
void JuleaReader::InitParameters()
{
    ...
    // if (m_Verbosity == 5)
    // {
    //     std::cout << "Julea Reader " << m_ReaderRank << " InitParameters()\n";
    // }
}
