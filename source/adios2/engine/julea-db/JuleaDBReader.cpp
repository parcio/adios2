...

{
    ...
    if (m_Verbosity == 5)
    {
        std::cout << "JuleaDB Reader " << m_ReaderRank << " Open(" << m_Name
...
JuleaDBReader::~JuleaDBReader()
{
    ...
    if (m_Verbosity == 5)
    {
        std::cout << "JuleaDB Reader " << m_ReaderRank << " deconstructor on "
                  << m_Name << "\n";
    }
}
                                  ...
                                  const float timeoutSeconds)
{
    ...
    {
        std::cout << "JuleaDB Reader " << m_ReaderRank
    ...
    m_IO.ResetVariablesStepSelection(false,
                                     "in call to JuleaDB Reader BeginStep");
...
void JuleaDBReader::EndStep()
{
    ...
    if (m_Verbosity == 5)
    {
        std::cout << "JuleaDB Reader " << m_ReaderRank << "   EndStep()\n";
    }
...
void JuleaDBReader::PerformGets()
{
    if (m_Verbosity == 5)
    {
        std::cout << "JuleaDB Reader " << m_ReaderRank << "     PerformGets()\n";
    }
...
void JuleaDBReader::Init()
{
    ...
    //     {
    //         throw std::invalid_argument(
    //             "ERROR: JuleaDBReader only supports OpenMode::Read from" +
    //             m_Name + " " + m_EndMessage);
    //     }
    // }
    // if (m_Verbosity == 5)
    // {
    //     std::cout << "JuleaDB Reader " << m_ReaderRank << " Init()\n";
    // }

    // TODO: which order?
    j_init();
    m_JuleaDBInfo = g_slice_new(JuleaDBInfo);
    m_JuleaDBInfo->semantics = j_semantics_new(J_SEMANTICS_TEMPLATE_DEFAULT);
    m_JuleaDBInfo->name_space = g_strdup(m_Name.c_str());
    // j_adios_init(m_JuleaDBInfo);
...
void JuleaDBReader::InitVariables()
{
    ...
    GetAllVarNamesFromKV(m_JuleaDBInfo->name_space, &names, &types, &count_names,
                         m_JuleaDBInfo->semantics);

    // std::cout << "++ JuleaDB Reader DEBUG PRINT: count_names " << count_names
    // << std::endl;
    ...
    for (unsigned int i = 0; i < count_names; i++)
    {
        ...
        std::cout << "JuleaDBReader names: " << names[i] << std::endl;
        GetVarMetadataFromKV(m_JuleaDBInfo->name_space, names[i], metadata,
                             m_JuleaDBInfo->semantics);

        // std::cout << "JuleaDBReader metadata address: " << (void*) metadata <<
        // std::endl; std::cout << "++ JuleaDB Reader DEBUG PRINT: shape_size: "
        // << metadata->shape_size << std::endl; std::cout << "++ JuleaDB Reader
        // DEBUG PRINT: start_size: " << metadata->start_size << std::endl;
        // std::cout << "++ JuleaDB Reader DEBUG PRINT: count_size: " <<
        ...
        if (metadata->shape_size > 0)
        {
            ...
            shape = shape2;
            // std::cout << "++ JuleaDB Reader DEBUG PRINT: shape" << std::endl;
        }
        if (metadata->start_size > 0)
        {
            start = start2;
            // start.front() = *metadata->start;
            // std::cout << "++ JuleaDB Reader DEBUG PRINT: start" << std::endl;
        }
        if (metadata->count_size > 0)
        {
            count = count2;
            // count.front() = *metadata->count;
            // std::cout << "++ JuleaDB Reader DEBUG PRINT: count" << std::endl;
        }
...
//     }
        ...
        case INT32:
            m_IO.DefineVariable<int32_t>(names[i], shape, start, count,
                                         constantdims);
            std::cout << "++ JuleaDB Reader m_IO.DefineVariable() for: "
                      << names[i] << std::endl;
            break;
        case UINT32:
    ...
    if (m_Verbosity == 5)
    {
        std::cout << "JuleaDB Reader " << m_ReaderRank << " InitVariables()\n";
    }
}
...
void JuleaDBReader::InitTransports()
{
    ...
    if (m_Verbosity == 5)
    {
        std::cout << "JuleaDB Reader " << m_ReaderRank << " InitTransports()\n";
    }
}
...
void JuleaDBReader::DoClose(const int transportIndex)
{
    if (m_Verbosity == 5)
    {
        std::cout << "JuleaDB Reader " << m_ReaderRank << " Close(" << m_Name
                  << ")\n";
    }
    g_free(m_JuleaDBInfo->name_space);
    g_slice_free(JuleaDBInfo, m_JuleaDBInfo);
}
