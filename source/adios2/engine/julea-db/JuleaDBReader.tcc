                                ...
                                std::string *data)
{
    ...
    std::cout << "JuleaDB Reader " << m_ReaderRank << " Namespace of variable "
              << m_Name << std::endl;
    // std::cout << "++ JuleaDB Reader DEBUG PRINT data_size %d" <<
    // metadata->data_size << std::endl;    //FIXME
    GetVarDataFromJuleaDB(name_space, metadata->name, metadata->data_size,
                        (void *)(data), batch);
    ...
    if (m_Verbosity == 5)
    {
        std::cout << "JuleaDB Reader " << m_ReaderRank << "     GetSync("
                  << variable.m_Name << ")\n";
    }
    j_batch_unref(batch);
}
...
void JuleaDBReader::GetSyncCommon(Variable<T> &variable, T *data)
{
    ...
    GetVarMetadataFromKV(m_JuleaDBInfo->name_space, metadata->name, metadata,
                         m_JuleaDBInfo->semantics); // FIXME

    GetVarDataFromJuleaDB(name_space, metadata->name, metadata->data_size,
                        (void *)(data), batch);
    // std::cout << "++ JuleaDB Reader DEBUG PRINT data_size: " <<
    // metadata->data_size << std::endl;    //FIXME
...
void JuleaDBReader::GetDeferredCommon(Variable<T> &variable, T *data)
{
    ...
    if (m_Verbosity == 5)
    {
        std::cout << "JuleaDB Reader " << m_ReaderRank << "     GetDeferred("
                  << variable.m_Name << ")\n";
    }
