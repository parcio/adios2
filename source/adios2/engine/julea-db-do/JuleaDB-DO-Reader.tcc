...
#ifndef ADIOS2_ENGINE_JULEADB_DO_READER_TCC_
#define ADIOS2_ENGINE_JULEADB_DO_READER_TCC_
...
template <class T>
void JuleaDB_DO_Reader::GetSyncCommon(Variable<T> &variable, T *data)
{
    if (m_Verbosity == 5)
    {
...
template <class T>
void JuleaDB_DO_Reader::GetDeferredCommon(Variable<T> &variable, T *data)
{
    if (m_Verbosity == 5)
    {
...
template <class T>
void JuleaDB_DO_Reader::ReadVariableBlocks(Variable<T> &variable)
{
    if (m_Verbosity == 5)
    {
...
JuleaDB_DO_Reader::AllStepsBlocksInfo(const core::Variable<T> &variable) const
{
    std::map<size_t, std::vector<typename core::Variable<T>::Info>>
        allStepsBlocksInfo;
    ...
    const std::vector<size_t> &blocksIndexOffsets, size_t step) const
{
    ...
    for (size_t i = 0; i < blocksIndexOffsets.size(); i++)
    {
        ...
        typename core::Variable<T>::Info info =
            ...
            *DB_DO_GetBlockMetadata(variable, entryID);
...
JuleaDB_DO_Reader::AllRelativeStepsBlocksInfo(
...
JuleaDB_DO_Reader::InitVariableBlockInfo(core::Variable<T> &variable, T *data)
{
    if (m_Verbosity == 5)
    {
    ...
    if (variable.m_SelectionType == SelectionType::WriteBlock)
    {
        const std::vector<typename core::Variable<T>::Info> blocksInfo =
            JuleaDB_DO_Reader::BlocksInfo(variable, stepsStart);
...
template <class T>
void JuleaDB_DO_Reader::SetVariableBlockInfo(
    ...
    typename core::Variable<T>::Info &blockInfo) const
{
...

    {
        ...
        typename core::Variable<T>::Info info =
            *DB_DO_GetBlockMetadata(variable, blockIndexOffset);
