...
#ifndef ADIOS2_ENGINE_JuleaDBDAIREADER_TCC_
#define ADIOS2_ENGINE_JuleaDBDAIREADER_TCC_
...
template <class T>
void JuleaDBDAIReader::GetSyncCommon(Variable<T> &variable, T *data)
{
    if (m_Verbosity == 5)
    {
...
template <class T>
void JuleaDBDAIReader::GetDeferredCommon(Variable<T> &variable, T *data)
{
    if (m_Verbosity == 5)
    {
...
template <class T>
void JuleaDBDAIReader::ReadBlock(Variable<T> &variable, T *data, size_t blockID)
{
    if (m_Verbosity == 5)
    {
        std::cout << "\n ReadBlock" << std::endl;
    ...
    DAIGetCountFromBlockMetadata(nameSpace, variable.m_Name, step, blockID, &count,
...
template <class T>
void JuleaDBDAIReader::ReadVariableBlocks(Variable<T> &variable)
{
    if (m_Verbosity == 5)
    {
...
JuleaDBDAIReader::AllStepsBlocksInfo(const core::Variable<T> &variable) const
{
    std::map<size_t, std::vector<typename core::Variable<T>::Info>>
        allStepsBlocksInfo;
                                ...
                                size_t step) const
{
    ...
    for (size_t i = 0; i < blocksIndexOffsets.size(); i++)
    {
        ...
        typename core::Variable<T>::Info info =
            ...
            *DAIDBGetBlockMetadata(variable, entryID);
...
JuleaDBDAIReader::AllRelativeStepsBlocksInfo(
...
JuleaDBDAIReader::InitVariableBlockInfo(core::Variable<T> &variable, T *data)
{
    if (m_Verbosity == 5)
    {
    ...
    if (variable.m_SelectionType == SelectionType::WriteBlock)
    {
        const std::vector<typename core::Variable<T>::Info> blocksInfo =
            JuleaDBDAIReader::BlocksInfo(variable, stepsStart);
...
template <class T>
void JuleaDBDAIReader::SetVariableBlockInfo(
    ...
    typename core::Variable<T>::Info &blockInfo) const
{
...

    {
        ...
        typename core::Variable<T>::Info info =
            *DAIDBGetBlockMetadata(variable, blockIndexOffset);
...
#endif // ADIOS2_ENGINE_JuleaDBDAIREADER_TCC_
